...
export interface GetTasksFilters {
    ...
    assignerId?: string
    limit?: number
    cursor?: string // created_at ISO string — fetch tasks older than this
}
...
export async function getTasks(filters?: GetTasksFilters): Promise<GetTasksResult> {
    const supabase = getSupabase()
    ...
    // Build main query — fetch limit+1 to know if there's a next page
    ...
    const allTaskIds = page.map(t => t.id)
...
export async function getTasksAssignedTo(userId: string): Promise<TaskWithUsers[]> {
    const { tasks } = await getTasks({ assigneeId: userId })
    return tasks
}

export async function getTasksCreatedBy(userId: string): Promise<TaskWithUsers[]> {
    const { tasks } = await getTasks({ assignerId: userId })
    return tasks
}
...
export async function changeStatus(
    ...
    // Clear on_hold_reason when not on_hold
