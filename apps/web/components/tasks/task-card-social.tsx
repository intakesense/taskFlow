...
import { TASK_STATUS_CONFIG, TASK_PRIORITY_CONFIG } from '@/lib/constants/task'
...
export function TaskCardSocial({
  ...
  const handleOnHoldConfirm = () => {
    onStatusChange?.(task.id, 'on_hold', onHoldReason.trim())
  ...
  return (
    ...
    >
      ...
      <Dialog open={onHoldDialogOpen} onOpenChange={(open) => !open && handleOnHoldCancel()}>
        <DialogContent>
          <DialogHeader>
          ...
          />
          <DialogFooter>
            <Button variant="outline" onClick={handleOnHoldCancel}>Cancel</Button>
