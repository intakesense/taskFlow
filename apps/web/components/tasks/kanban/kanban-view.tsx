...
// Jira/Trello-style drop animation
const dropAnimation = {
  duration: 250,
...
export function KanbanView({
  ...
  const handleDragOver = useCallback((event: DragOverEvent) => {
    ...
    // Check if hovering over another task - get its column
    const overTask = tasks.find(t => t.id === overId)
    if (overTask) {
      setActiveColumnId(overTask.status as TaskStatus)
    }
  }, [tasks])
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (targetStatus && targetStatus !== task.status) {
      // If moving to on_hold, show dialog to get reason
      ...
      <Dialog open={!!pendingOnHold} onOpenChange={(open) => !open && handleOnHoldCancel()}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Please provide a reason for putting this task on hold. This helps
              the team understand why work has been paused.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            value={onHoldReason}
