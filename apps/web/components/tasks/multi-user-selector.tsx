...
import { EMPLOYEE_LEVEL_COLORS } from '@/lib/constants'
...
export function MultiUserSelector({
  ...
  const getLevelBadgeColor = (level: number) => {
    return EMPLOYEE_LEVEL_COLORS[level] || 'bg-gray-500'
  }
            ...
            {isLoading ? (
              ...
              <div className="py-1">
                {filteredUsers.map((user) => {
                  ...
                  const isDisabled = !isSelected && !!maxSelections && selectedUserIds.length >= maxSelections
                  return (
                    <button
                      key={user.id}
                      type="button"
