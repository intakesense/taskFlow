...
export interface ConversationWithMembers extends Conversation {
    members: UserBasic[]; // Convenience: user objects only
    membersWithStatus: ConversationMemberWithUser[]; // Full: includes last_read_at, joined_at
...
export interface TaskAuditLogWithUser extends TaskAuditLog {
    user: UserBasic | null;
}
