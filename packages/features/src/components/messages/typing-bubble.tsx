...
import type { UserBasic } from '@taskflow/core';
import { typingBubbleVariants } from '@taskflow/ui';
...
export function TypingBubble({ typingUsers }: TypingBubbleProps) {
  ...
  return (
    ...
    >
      ...
      {/* Text indicator */}
      <span className="text-xs text-muted-foreground mt-2 flex-1">{displayText}</span>
    </m.div>
  );
}
