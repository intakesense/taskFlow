...
import type { GroupedReaction } from '@taskflow/core';
...
export function ReactionBadges({ reactions, onToggle, isOwn }: ReactionBadgesProps) {
  ...
  return (
    <TooltipProvider delayDuration={400}>
      <div
        className={cn('flex flex-wrap gap-1.5 mt-1.5', isOwn ? 'justify-end' : 'justify-start')}
      >
            ...
            >
              <Tooltip>
                ...
                <TooltipContent side="top" className="max-w-[200px] hidden sm:block">
                  <p className="text-xs">{users.map((u) => u.name).join(', ')}</p>
                </TooltipContent>
              </Tooltip>
            </m.div>
          ))}
...
export function QuickReactionsBar({ onSelect, currentEmoji, onClose }: QuickReactionsBarProps) {
...
export function MessageActions({ onReact, onReply, isOwn }: MessageActionsProps) {
  return (
    <div
      className={cn(
