...
export function useAllProgressFeed(limit: number = 50) {
  ...
  return useQuery({
    ...
    queryFn: () => progress.getAllProgressUpdates(limit),
...
export function useTaskProgress(taskId: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: () => progress.getProgressUpdates(taskId!),
    enabled: !!taskId,
    staleTime: STALE_TIME.MESSAGES,
  });
}
...
export function useTaskProgressByDate(taskId: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: () => progress.getProgressUpdatesByDate(taskId!),
    enabled: !!taskId,
    staleTime: STALE_TIME.MESSAGES,
  });
}
...
export function useCreateProgressUpdate() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      content: string;
      sender?: UserBasic;
    }) => progress.createProgressUpdate(taskId, senderId, content),
    ...
    onError: (error, variables, context) => {
      ...
      if (context?.previousProgress) {
        queryClient.setQueryData(taskProgressKeys.task(variables.taskId), context.previousProgress);
      }
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.setQueryData<ProgressUpdateWithComments[]>(
        ...
        (old = []) =>
          old.map((update) =>
            update.id.startsWith('temp-') ? { ...data, comments: [], commentCount: 0 } : update
          )
      );
...
export function useAddProgressComment() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      content: string;
      sender?: UserBasic;
    }) => progress.addProgressComment(progressId, taskId, senderId, content),
    onMutate: async (variables) => {
      await queryClient.cancelQueries({
        queryKey: taskProgressKeys.task(variables.taskId),
      });
      ...
      queryClient.setQueryData<ProgressUpdateWithComments[]>(
        ...
        (old = []) =>
          old.map((update) => {
            ...
            return {
              ...update,
              comments: update.comments.map((c) => (c.id.startsWith('temp-') ? data : c)),
            };
          })
      );
      queryClient.invalidateQueries({
